'''
This is the torch version of the action builder
'''
...
from katacr.classification.elixir.torch_predict_elixir import ElixirClassifier, args
elixir_args=args
from katacr.constants.card_list import card2elixir, eng2chi, eng_names
import cv2
from pathlib import Path

OCR_TEXT_SIZE = (150,50)  # bottom center = elixir top center
WRONG_CARD_FRAME_DELTA = 10  # 10 * 0.1 = 1 sec
EMPTY_CARD_UPDATE_FRAME_DELTA = 3  # 5 * 0.1 = 0.5 sec, delay to use last classification result after card empty
...
class ActionBuilder:
  def __init__(self, persist: int=2, ocr: OCR = None):
    ...
    self.eng2chi = eng2chi
    self.eng_names = eng_names
    self.ocr = OCR() if ocr is None else ocr
    self.elixir_classifier = ElixirClassifier("C:\Disk_D\RL_Finance\KataCR\elixir_resnet.pth", elixir_args)
  ...
  def _update_cards(self):
    ...
    # print(f"Action(Time={self.time},frame={self.frame_count}): {self.cards=}, {self.cards_memory=}, {self.deploy_cards=}")
    #   # DEBUG
  def _add_action(self, elixir_box, card_name):
    if card_name not in self.cards_memory:
        print(f"Warning(action): card '{card_name}' not found in cards_memory {self.cards_memory}, using default action.")
        self.actions.put({'xy': None, 'card_id': 0, 'offset': 0})
    else:
        b = elixir_box
        xyxy = b[:4]
  ...
  def _ocr_text(self, elixir_box):
    ...
    text_xyxy = np.array([xy[0]-size[0]/2, xy[1]-size[1]-30, xy[0]+size[0]/2, xy[1]-30], np.int32)
    ...
    for result in results:
      ...
      for name in self.deploy_cards.copy():
          if edit_distance(result['text'], self.eng2chi[name], dis='s1') <= 1:
            return(name)
  ...
  def _find_action(self):
    ...
    for b in elixir:
      ...
      if class_name == True:  # and len(self.deploy_cards) == 1:
        avail_cards = [c for c in self.eng_names if card2elixir[c] == -elixir_num]
      ...
      print(class_name)
      if class_name in self.deploy_cards:
