...
from katacr.constants.card_list import card2idx, unit2cards, eng2chi
...
class StateBuilder:
  ...
  def __init__(self, persist: int = 3, ocr = None):
    """
    Args:
    ...
    self.ocr = CnOcr() if ocr is None else ocr
    self.reset()
  ...
  def _find_text_info(self, deploy_cards):
    ...
    if result is not None:
      for detect in result:
          if len(detect['text']) < 2: continue
          for name in self.deploy_history:
              if edit_distance(detect['text'], eng2chi[name], dis='s1') <= EDIT_DISTANCE_THRE:
                      center_x = (detect['position'][2][0] + detect['position'][3][0]) // 2
                      center_y = (detect['position'][2][1] + detect['position'][3][1]) // 2
                      text_info.append((card2idx[name], center_x, center_y))
