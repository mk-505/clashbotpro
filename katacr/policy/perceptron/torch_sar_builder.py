...
class SARBuilder:
  def __init__(self, verbose=True):
    ...
    from katacr.policy.perceptron.torch_state_builder import StateBuilder
    from katacr.policy.perceptron.torch_action_builder import ActionBuilder
    from katacr.policy.perceptron.torch_reward_builder import RewardBuilder
    if verbose:
      print("Building SAR builder...")
    self.visual_fusion = VisualFusion(args)
    self.state_builder = StateBuilder(ocr=None)
    self.action_builder = ActionBuilder(ocr=None)
    self.reward_builder = RewardBuilder(ocr=None)
  ...
  def render(self, action):
    img = self.state_builder.render(action)
