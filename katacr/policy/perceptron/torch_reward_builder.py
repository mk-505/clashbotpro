import sys
sys.path.append('C:/Disk_D/RL_Finance/KataCR')
import numpy as np
...
from katacr.classification.torch_predict import args
import re, os
from collections import deque
from katacr.ocr_text.cnocr_predict import OCR
...
## XYXY_TOWER_BAR_NUM = [(0.26, 0.3, 0.8, 1.0), (0.26, 0.0, 0.8, 0.7)]
XYXY_TOWER_BAR_NUM = [(0.26, 0.3, 0.8, 1.1), (0.26, 0.0, 0.8, 0.7)]

XYXY_KING_TOWER = [(185, 700, 385, 885), (195, 0, 385, 200)]
XYXY_TOWER = [[(50, 625, 175, 770), (400, 625, 525, 770)], [(50, 135, 175, 285), (405, 135, 525, 285)]]
OCR_NUM_CONF_THRE = 0.3
DESTROY_FRAME_DELTA_THRE = 5
MAX_DELTA_HP = 1600
...
class RewardBuilder:
  def __init__(self, ocr: OCR = None):
    ## self.ocr = OCR() if ocr is None else ocr
    self.reset()
  ...
  def reset(self):
    ...
    self.frame_count = 0
    self.time = 0
    self.tower_hp_buffer = [[deque(maxlen=3) for _ in range(2)] for _ in range(2)]
    self.hp_fixed_flag = np.full((2, 2), False, dtype=bool)


  def _ocr_hp(self, xyxy, target_size):
    img = extract_img(self.img, xyxy, target_size=target_size)
    ocr_single_line=CnOcr()
    #cv2.imshow("Extracted Image", img)
    #cv2.waitKey(0)
    #cv2.destroyAllWindows()

    ## results = self.ocr(img)
    results = ocr_single_line.ocr_for_single_line(img)
    #print(results)
    if results['text'] == '': return -1

    score=float(results['score'])
    if score < OCR_NUM_CONF_THRE:
        print(f"Wrong: (time={self.time}) as the conf={score:.4f} is too low")
        num = -1
    else:
        num = int(re.sub(r'\D', '', results['text']) or -1)
    return num
  ...
  def _has_other_item_cover(self, xyxy):
    ...
    boxes = []
    for name in ['emote', 'clock', 'elixir', 'bar-level', 'bar']:
  ...
  def get_reward(self, verbose=False):
    ...
    if len(king_tower_bar_box):
      for b in king_tower_bar_box:
        ...
        if self.hp_king_tower[bel] != -1:
          delta = hp-self.hp_king_tower[bel]
          if delta > 0:
    ...
    for b in tower_bar_box:
      ...
      if self._has_other_item_cover(b[:4]): 
        ## print("True")
        continue
      xyxy = xyxy2sub(b[:4], XYXY_TOWER_BAR_NUM[int(b[-1])])
      hp = self._ocr_hp(xyxy, TARGET_SIZE_TOWER_BAR)

      self.tower_hp_buffer[i][j].append(hp)
      
      ## check if we have 3 successive equal values
      buffer = self.tower_hp_buffer[i][j]
      stable_hp = (len(buffer) == 3 and all(v == buffer[0] and v != -1 for v in buffer))

      if self.hp_tower[i,j] != -1:
        delta=hp-self.hp_tower[i,j]
        if delta > 0:
            if stable_hp:
                print(f"[Fix] Tower ({i},{j}): Accepting higher stable HP={hp}")
                hp = buffer[0]  # accept the consistent high value
                self.hp_fixed_flag[i, j] = True
            else:
                print(f"Warning(reward): (time={self.time}) tower id={b[-4]}, ocr_hp={hp} > old hp={self.hp_tower[i,j]}, suspicious increase")
                hp = -1
        elif abs(delta) > MAX_DELTA_HP:
          print(f"Warning(reward): (time={self.time}) tower id={b[-4]}, old_hp-ocr_hp={delta}'s abs > {MAX_DELTA_HP=}, ignore it")
          hp = -1
      elif stable_hp:
        # Even if previous was -1, but we now have a stable value — accept it
        print(f"[Fix] Tower ({i},{j}): Previously -1, now stable HP={buffer[0]}")
        hp = buffer[0]
        ## self.hp_fixed_flag[i, j] = True
    ...
    for i, (olds, nows) in enumerate(zip(self.hp_tower, now_hp_tower)):
      ...
      for j, (old, now) in enumerate(zip(olds, nows)):
        if self.hp_fixed_flag[i, j] and now != -1:
          self.hp_tower[i, j] = now  # Only update if it's a valid detection
          self.hp_fixed_flag[i, j] = False
          continue  # Skip reward calc
