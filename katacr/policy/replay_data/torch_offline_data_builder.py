...
class OfflineDatasetBuilder:
  def __init__(self, compress_threads=8):
    ...
    self.path_save_result = path_root / f"logs/offline" / time.strftime("%Y.%m.%d_%H-%M-%S")
    self.save_episode_path = self.path_save_result / "episodes"
  ...
  def process(
    ...
    self.save_data(self.save_episode_path)
