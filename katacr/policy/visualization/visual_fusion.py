...
from katacr.yolov8.torch_combo_detect import ComboDetector, path_detectors
#from katacr.ocr_text.paddle_ocr import OCR
from katacr.ocr_text.cnocr_predict import OCR
from katacr.classification.torch_predict import CardClassifier, args
...
path_classifier = "C:\Disk_D\RL_Finance\KataCR\card_resnet.pth"
...
class VisualFusion_version0:
  def __init__(self, args):
    self.ocr = OCR()
    self.yolo = ComboDetector(path_detectors, show_conf=True)
    self.classifier = CardClassifier(path_classifier, args)
  ...
  def process(self, x, pil=False):
    ...
    time = self.ocr.process_part1(parts[0])
    arena = self.yolo.infer(parts[1], pil=pil)

    cards = self.classifier.process_part3(parts[2])
    elixir = self.ocr.process_part3_elixir(parts[2])
    self.info = dict(
...
## 2 vf versions to use, decide later on...
...
class VisualFusion:
  ...
  def process(self, x, pil=False):
    ...
    parts_pos = np.array(parts_pos)
    self.parts_pos = (parts_pos.reshape(-1, 2) * np.array(x.shape[:2][::-1])).astype(np.int32).reshape(-1, 4)
    time = self.ocr.process_part1(parts[0],parts[3])
    arena = self.yolo.infer(parts[1], pil=pil)
